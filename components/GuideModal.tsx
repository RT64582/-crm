...
async function updateCRM(analysisResult) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="relative mx-auto p-6 border w-full max-w-3xl shadow-xl rounded-xl bg-white" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center border-b border-slate-200 pb-4">
          <h3 className="text-xl leading-6 font-bold text-slate-800">מדריך לחיבור ל-CRM</h3>
        ...
        <div className="mt-4 space-y-5 text-right p-2 max-h-[70vh] overflow-y-auto">
            <div>
                <h4 className="font-semibold text-slate-700 text-lg">מבוא</h4>
            ...
            <div>
                <h4 className="font-semibold text-slate-700 text-lg">שלב 1: ייצוא ידני (ללא קוד)</h4>
                <p className="text-slate-600 text-sm mt-1 leading-relaxed">
            ...
            <div>
                <h4 className="font-semibold text-slate-700 text-lg">שלב 2: אינטגרציה אוטומטית (למפתחים)</h4>
                <p className="text-slate-600 text-sm mt-1 leading-relaxed">
                   זרימת עבודה אוטומטית דורשת שירות צד-שרת (backend) שישמש כגשר בין ניתוח ה-AI ל-API של ה-CRM.
                   <br/>
                   <strong className="font-semibold">תרשים זרימה:</strong> 
                   <code className="text-xs">קובץ שמע/טקסט ➔ ניתוח ב-Gemini API ➔ שירות ה-Backend שלכם ➔ קריאת API ל-CRM (למשל, יצירת משימה)</code>.
                </p>
                 <p className="text-slate-600 text-sm mt-3 font-semibold">דוגמת קוד (Pseudo-code ב-Node.js):</p>
                 <pre className="text-sm mt-2 bg-slate-800 text-white p-4 rounded-md max-h-60 overflow-y-auto whitespace-pre-wrap font-mono text-left dir-ltr border border-slate-700">
                  <code>
                    {codeExample.trim()}
                  </code>
                </pre>
            ...
            <div>
                <h4 className="font-semibold text-slate-700 text-lg">הערות אבטחה חשובות</h4>
                 <div className="bg-yellow-50 border-r-4 border-yellow-400 text-yellow-800 p-4 my-2 rounded-l-lg text-sm">
             ...
             <div>
                <h4 className="font-semibold text-slate-700 text-lg">כתב ויתור</h4>
